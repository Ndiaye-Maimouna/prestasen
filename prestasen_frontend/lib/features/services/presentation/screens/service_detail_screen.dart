...
import '../../../../shared/widgets/contact_service_sheet.dart';
...
class ServiceDetailScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: serviceAsync.when(
        ...
        error: (error, _) => const Center(child: Text('Service introuvable')),
        data: (service) {
          ...
          final estProprietaire = monProfilAsync.when(
            data: (p) => p.id == service.professionnelId,
            loading: () => null,
          ...
          return Stack(
            children: [
                ...
                slivers: [
                  SliverAppBar(
                    ...
                    flexibleSpace: FlexibleSpaceBar(
                      background: ImageGallery(images: service.images, height: 280),
                    ),
                  ),
                  SliverToBoxAdapter(
                    child: Padding(
                      padding: EdgeInsets.fromLTRB(
                        ...
                        AppSizes.lg,
                        AppSizes.lg,
                        estProprietaire == true ? AppSizes.lg : 120, // moins de padding bas si pas de bouton flottant
                      ),
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(service.titre, style: Theme.of(context).textTheme.headlineMedium),
                          ...
                          _CarteProfessionnel(
                            nom: service.professionnelNom,
                            label: 'Proposé par',
                            navigable: !masquerNavigationProfessionnel,
                            onTap: () => context.push('/professionnels/${service.professionnelId}'),
                          ),
              ...
              // Correctif : on n'affiche le bouton "Faire une demande" que lorsqu'on sait
              // avec certitude (estProprietaire == false) que l'utilisateur n'est pas
              // propriétaire du service. Tant que le statut est inconnu (null, en chargement),
              // on n'affiche rien, au lieu d'afficher le bouton puis de le faire disparaître.
              if (estProprietaire == false) _boutonFaireUneDemande(context, ref, service),
            ],
          );
        },
      ),
  ...
  Widget _boutonFaireUneDemande(BuildContext context, WidgetRef ref, dynamic service) {
    return Positioned(
      left: AppSizes.lg,
      right: AppSizes.lg,
      bottom: AppSizes.lg,
      child: PrimaryButton(
        ...
        onPressed: () async {
          final professionnel =
          await ref.read(profilProfessionnelDetailProvider(service.professionnelId).future);
          if (context.mounted) {
            ContactServiceSheet.afficher(
              context,
